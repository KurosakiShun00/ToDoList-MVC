...
public class ToDoListsController : Controller
{
    ...
    public async Task<IActionResult> Create(ToDoListsCreateViewModel viewModel)
    {
        ...
        if (listNumber >= 30)
        {
            ...
            ViewData["ErrorMessage"] =
                "Non è possibile creare un'altra lista in quanto si è raggiunto il limite massimo di 30 liste.";
    ...
    public async Task<IActionResult> DuplicateList(int id)
    {
        ...
        foreach (var toDo in list.ToDos) await _service.AddToDoToListAsync(newList.Id, toDo, userId);
    ...
    public async Task<IActionResult> DeleteConfirmed(int id)
    {
        ...
        var isDeleted = await _service.DeleteListAsync(id, userId);
    ...
    public async Task<IActionResult> EditToDo(int id, ToDoEditViewModel viewModel)
    {
        ...
        var redirectId = viewModel.ToDoListId != 0 ? viewModel.ToDoListId : existingToDo.ToDoListId;
    ...
    public async Task<IActionResult> ExportList(int id)
    {
        ...
        stringBuilder.Append("DELL'UTENTE: ")
            .Append(User.FindFirstValue(ClaimTypes.GivenName) ?? "nome non trovato".ToUpper()).AppendLine();
        stringBuilder.Append("E-MAIL: ").Append(User.FindFirstValue(ClaimTypes.Email) ?? "e-mail non trovata")
            .AppendLine();
        ...
        if (list.ToDos.Count == 0) stringBuilder.Append("LA LISTA E' VUOTA");
            ...
            foreach (var toDo in list.ToDos)
                stringBuilder.Append(toDo.Name + ", ")
                    .Append((toDo.IsCompleted ? "Completata" : "Non Completata") + ", ").Append(toDo.CategoryId)
                    .AppendLine();

        var str = stringBuilder.ToString();

        var fileBytes = Encoding.UTF8.GetBytes(str);

        var fileName = $"{list.Name ?? "UnnamedList".Replace(" ", "_")}_Export.txt";
    ...
    public async Task<IActionResult> ImportList(IFormFile file)
    {
        ...
        using (var reader = new StreamReader(file.OpenReadStream()))
        {
            ...
            while ((linea = await reader.ReadLineAsync()) != null)
            {
                ...
                var nomeToDo = parti[0].Trim();
                var stato = parti[1].Trim();
                var categoria = parti[2].Trim();

                var isCompleted = stato.Equals("Completata", StringComparison.OrdinalIgnoreCase);

                int? categoryId = int.TryParse(categoria, out var parsedId) ? parsedId : null;
        ...
        foreach (var toDo in toDos) await _service.AddToDoToListAsync(newList.Id, toDo, userId);
    ...
    public async Task<IActionResult> GetCalendarEvents()
    {
        ...
        if (todos == null) return Json(new List<object>());
